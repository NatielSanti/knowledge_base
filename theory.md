...
- API на основе HTTP можно тестировать в браузере, используя, к примеру, расширение Postman, 
или в командной строке с помощью `curl` (при условии, 
что вы применяете `JSON` или другой текстовый формат).
...
без промежуточного звена для буферизации сообщений, 
они оба должны работать на протяжении всего обмена данными.
- Клиенты должны знать местонахождение (URL) экземпляра (-ов) сервиса. 
- Это нетривиальная проблема для современных приложений. 
Для определения местонахождения экземпляров сервисов клиентам приходится использовать 
так называемый механизм обнаружения сервисов.
...
## TCP / IP, UDP, Http, Socket, разница XMPP

### UDP:
UDP - это служба протокола пользовательских данных без установления соединения
(протокол пользовательских данных), которая может взаимодействовать с сервером без
необходимости знать IP-адрес и порт прослушивания, и нет необходимости устанавливать
связь без цели Socket , доставляет только дейтаграмму, независимо от того,
успешно ли ее получает получатель. Следовательно, это ненадежная передача и может
привести к потере пакетов данных. Однако из-за этих характеристик эффективность
передачи выше, чем у TCP. Например, передача QQ
### TCP:
TCP - это протокол управления передачей с установлением соединения
(протокол contorl преобразования). Он должен взаимодействовать с сервером.
Он отличается высокой безопасностью и надежностью. Он требует трехстороннего
установления связи с сервером. Задержка перегрузки сети. Например, передача MSN

TCP / IP на самом деле представляет собой набор протоколов,
который включает сотни протоколов с различными функциями, такими как удаленный вход,
передача файлов, электронная почта и т. д., в то время как протокол TCP и IP Протокол
- это два основных и важных протокола, обеспечивающих полную передачу данных.
- Вообще говоря, TCP / IP - это семейство интернет-протоколов, а не только TCP и IP.

### Socket:
socket похож на внешний API для TCP и UDP. Существует два режима работы с подключением:
ориентированный на подключение (TCP) и ориентированный на подключение (UDP).
При использовании UDP нет необходимости указывать место назначения сокета,
но при использовании TCP необходимо указать назначение сокета.
Требуется предварительное соединение, в противном случае соединение будет невозможно.

#### Преимущество:
1. Данные передачи являются байтовыми, данные передачи можно настраивать, 
а объем данных небольшой. Соответствующая разработка мобильного терминала, 
низкие затраты на мобильную связь
2. Короткое время передачи данных и высокая производительность.
3. Подходит для обмена информацией в реальном времени между C / S
4. Может быть зашифрован, высокая безопасность данных

#### Недостатки:
1. Переданные данные необходимо проанализировать и 
преобразовать в данные уровня приложения.
2. Высокие требования к уровню развития разработчиков.
3. По сравнению с передачей протокола Http объем разработки был увеличен.
   
#### Применимая сцена:
Онлайн-игры, взаимодействие с банком, оплата.

### HTTP：
Протокол HTTP - это протокол передачи гипертекста (протокол передачи гипертекста), 
который является основой веб-сетей и одним из широко используемых протоколов для 
сетей мобильных телефонов. Протокол HTTP - это приложение, 
построенное на основе протокола TCP, поэтому, если вы поддержка HTTP, 
вы должны поддерживать передачу TCP. Общие методы запроса - это получение и публикация, 
а также веб-службы.
Самая примечательная особенность HTTP-соединения заключается в том, что каждый запрос, 
отправленный клиентом, требует, чтобы сервер отправил ответ. 
После завершения запроса соединение будет активно разорвано. 
...
1. Скорость передачи низкая, а пакет данных большой.
2. Если взаимодействие осуществляется в реальном времени, производительность сервера падает.
3. Низкая безопасность передачи данных.
   
#### Применимая сцена:
Компания OA service, Интернет-сервис

[ссылка](https://russianblogs.com/article/72661606423/)

[к оглавлению](#Theory)

## GRASP
GRASP (General Responsibility Assignment Software Patterns) — 
шаблоны проектирования, используемые для решения общих задач по назначению обязанностей классам и объектам.

#### Информационный эксперт (Information Expert)
объект, обладающий максимумом информацией, необходимой для выполнения назначенных обязанностей.

#### Низкая связанность (Low Coupling) и Высокое зацепление (High Cohesion)
В котором, каждая утилита (если ее рассматривать как модуль) 
выполняет лишь минимальные обязанности (высокое зацепление) и почти ничего не знает о природе других утилит 
(низкая связность), в связи с чем может быть легко заменена на аналог в некотором варианте использования.

#### Устойчивый к изменениям (Protected Variantions)
устранении точек неустойчивости, путем определения их в качестве интерфейсов и 
реализации для них различных вариантов поведения.

#### Контроллер (Controller)
Контроллер отвечает за обработку входных системных событий, 
делегируя обязанности по их обработке компетентным классам.

#### Полиморфизм (Polymorphism)

#### Чистая выдумка (Pure Fabrication)
Существует понятие модели программирования по предметной области, согласно которой, 
каждой сущности из предметной области соответствует один или более классов программной среды. 
При этом, обязанности взаимодействия сущностей, как правило накладываются на них самих. 
Такой подход имеет очевидный недостаток — высокая связность модулей системы. 
Шаблон чистая выдумка позволяет решить данную проблему, 
путем введения в программную среду дополнительного класса (не отражающего реальной сущности 
из предметной области) и наделение его требуемыми обязанностями.

#### Перенаправление (Indirection)

Шаблон перенаправление реализует низкую связность между классами, 
путем назначения обязанностей по их взаимодействию дополнительному объекту — посреднику.
