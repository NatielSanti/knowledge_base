...
+ [Detecting anomalies using Isolation Trees](architect.md#Detecting-anomalies-using-Isolation-Trees)
+ [Распиливание монолита](architect.md#Распиливание-монолита)
...
Использование Машинного обучения для выявления аномалий в работе сервисов

### Пример собираемых метрик
...
- Использовать `Сайдкар`(утилити сервис в поде кубернейта) сервисы для публикации и отслеживания метрик
- Использованеи исторических данных для определения "нормы"
- `Isolation Tree` это `Decision Tree`

[к оглавлению](architect.md#Architect)

## Распиливание монолита

- Мы создаём новую архитектуру целиком на микросервисах, а потом через гейтвей переключаем пользователя. 
Нужно написать всю микросервисную архитектуру сразу
- Новая функциональность пишется сразу в новых сервисах

### Советы при распиле 

- Каждый сервис создаёт сразу же его АПИ библиотеку
- Роутинг запросов
- CI/CD
- Различные методы связи между микросервисами (очередь, http, websocket)
- логирование (kafka, Lucene)

### Принципы

- Данные сервисов должны быть независимы. У каждого сервиса своя бд(таблицы), 
которые выступают источником правды
- Деление по бизнес ответственностям. Нужно ли делить функциональность на микросервисы?
  - АПИ будет использоваться разными сервисами?
  - Сервисы сильно связаны?
  - Это требование бизнеса?
- Микросервисная инфраструктура может быть дорогой, стоит ли оно того?
- от 2 до 4 человек на сервис

[к оглавлению](architect.md#Architect)

## Five common system design interview mistakes

- Не нужно интро. Н-р  объяснять о необходимости гейтвея
- Не нужно объяснять и делать рассчеты без необходимости. 
Н-р нагрузки и количества пользователей.
- Не знание протоколов и их применения. RTMP, HTTP, gRPC и т.д.
- Не говорить о технологиях, которых не знаешь
- Плюсы и минусы технологий
- Главное. Начинать с главное и идти к мелочам, а не заострять внимание на мелочах

[к оглавлению](architect.md#Architect)

## Заметки

- gRPC это HTTP 2.0 . Без блокирования через голову очереди. Например, если один жирный запрос стоит в голове очереди, 
он не будет блокировать всю систему. Multiplexing
- Для обнаружения сервиса можно использовать:
   - Kubernetes DNS / Service (в k8s)
   - либо Consul / Eureka в non-k8s окружениях
   
   Для передачи запросов:
   - HTTP/REST — стандартный вариант
   - gRPC — если важна производительность и строгие контракты
...
- API Gateway — это инфраструктурный компонент, который служит единой точкой входа, скрывает внутреннюю архитектуру микросервисов и берет на себя кросс-каттинг concerns, такие как аутентификация, rate limiting, маршрутизация и агрегация. Он не должен содержать бизнес-логику и должен оставаться stateless и масштабируемым.
- API Gateway решает задачи входящего трафика и взаимодействия с клиентами, тогда как Service Mesh управляет внутренним взаимодействием между сервисами, обеспечивая безопасность, наблюдаемость и контроль трафика. В зрелых микросервисных архитектурах они используются совместно.
- Service Discovery — это механизм динамического обнаружения сервисов, который позволяет микросервисам находить друг друга без жёстких зависимостей от сетевых адресов. Он может быть реализован на стороне клиента или сервера, а в Kubernetes чаще всего используется встроенный DNS и сервисы, иногда в сочетании с service mesh.
- Circuit Breaker — это паттерн отказоустойчивости, который предотвращает каскадные отказы, временно блокируя вызовы к нестабильному сервису и позволяя системе быстро деградировать и восстановиться. Он обычно используется вместе с таймаутами, retry и bulkhead.
