...
Приложения требуют `времени процессора`, `памяти`, `жесткого диска`, `устройства ввода и вывода`.
Чтобы избежать ситуацию, когда одно приложение 
поедает все ресурсы и остальным не хватает есть следующие механизмы:

### Виртуализация

Это операционная система, поверх операционной системы Virtual Machines.
Часто можно арендовать машину и запустить на неё несколько виртуальных машин для своих приложений.

- +не нужно заботить о компьютере носителе, это задача хостинга
- +платформонезависимосе решение. Я могу развернуть на платформе виндоус несколько виртуальных машин с линуксом
- +динамическое и статическое распределение ресурсов, можно просто запускать и останавливать VM, когда мне нужно
- -запуск VM это запуск полноценной операционной системы, которая сама по себе потребляет ресурсы

### Контейнеризация

Изоляция приложения, без необходимости запускать его на отдельной виртуальной машине
- +контейнеры быстро запускаются
- +платформонезависимость
- +статическое распределение ресурсов
- -проблемы с фаерволом, необходимо настраивать отдельно
- -иногда поддержка контейнеров дороже чем просто запуск приложения на основной ОС
