...
Stream — это одноразовый вычислительный pipeline, а не структура данных.
Он может быть backed by одноразовый или бесконечный источник, поэтому повторное использование запрещено на уровне API.
...
- Порядок зависит от encounter order источника
- Sequential stream сохраняет порядок, если источник ordered
- Parallel stream может сохранять порядок, но не для всех терминальных операций
- forEach — без гарантий, forEachOrdered — с сохранением порядка, но без превращения в sequential
- findFirst всегда уважает порядок, findAny — нет
- Операции выполняются лениво и per-element
- parallelStream(): НЕ создаёт новый executor всегда использует ForkJoinPool.commonPool()
- ForkJoinPool рассчитан на non-blocking задачи, если внутри sleep, lock, I/O - будет плохо, потому что блокирует worker, уменьшает parallelism, ломает work-stealing
- map выполняет one-to-one преобразование элементов,
- flatMap — one-to-many с последующим «схлопыванием» вложенных stream’ов в один.
- В parallelStream для collect каждый поток собирает элементы в свой mutable контейнер. Combiner отвечает за безопасное объединение этих контейнеров. Если combiner написан некорректно, данные теряются или возникает гонка. Supplier создаёт контейнер, accumulator добавляет элемент, combiner объединяет контейнеры.
- groupingBy создает обычный HashMap, который не потокобезопасен, поэтому parallelStream может потерять элементы.
- groupingByConcurrent использует ConcurrentHashMap, позволяет потокам безопасно добавлять элементы к одним ключам.
- LinkedHashMap сохраняет порядок, но при parallelStream его использование небезопасно — combiner не синхронизирован.
