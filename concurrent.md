...
- Виртуальная машина JVM автоматически распределяет нагрузку между потоками-носителями. Разработчик контролирует параллелизм, ограничивая доступ к общим ресурсам, разделяя исполнителей по рабочей нагрузке и применяя обратное давление. Виртуальные потоки масштабируют параллелизм, а не пропускную способность. лимиты обязательны, пул — это точка контроля, а не оптимизация
- Виртуальные потоки - это облегченные потоки, управляемые JVM, которые обеспечивают массовый параллелизм с использованием кода в стиле блокировки. Коррутины - это конструкции языкового уровня для совместной многозадачности. Loom обеспечивает масштабируемость, подобную сопрограмме, в Java без изменения модели программирования.
- Virtual Threads: ✅ HTTP серверы ✅ DB calls ✅ legacy blocking code ✅ microservices
- Coroutines: ✅ сложные async-пайплайны ✅ UI ✅ event-driven системы
- ForkJoinPool ✅ CPU-bound ✅ рекурсивные алгоритмы параллельные вычисления ✅ обработка массивов, графов, деревьев
